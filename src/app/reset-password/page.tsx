...
function ResetForm() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (j.success) { setDone(true); setTimeout(() => router.push('/login'), 2500); }
  ...
  return (
    <>
      ...
      <p className="text-sm text-zinc-500 text-center mt-6">
        <Link href="/forgot-password" className="text-violet-600 dark:text-violet-400 hover:text-violet-500 dark:hover:text-violet-300 transition-colors">
          Request a new link
        </Link>
      </p>
    </>
