...
function ForgotForm() {
  ...
  const clientId = params.get('clientId') ?? process.env.NEXT_PUBLIC_AUTH_CLIENT_ID ?? '';
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!clientId) { setError('Missing client ID. Please use the reset link from your application.'); return; }
  ...
  if (sent) return (
    <div className="text-center space-y-4">
      ...
      <h1 className="text-xl font-bold tracking-tight text-zinc-900 dark:text-white">Check your email</h1>
      <p className="text-sm text-zinc-500 dark:text-zinc-400">
        If <span className="text-zinc-800 dark:text-zinc-200 font-medium">{email}</span> is registered, you'll receive a reset link within a minute.
      </p>
      <Link href="/login" className="text-sm text-violet-600 dark:text-violet-400 hover:text-violet-500 dark:hover:text-violet-300 transition-colors block">
        ← Back to sign in
      </Link>
    </div>
  );
  ...
  return (
    <>
      <h1 className="text-xl font-bold tracking-tight text-zinc-900 dark:text-white text-center mb-1">Forgot password?</h1>
      ...
      <p className="text-sm text-zinc-500 text-center mt-6">
        <Link href="/login" className="text-violet-600 dark:text-violet-400 hover:text-violet-500 dark:hover:text-violet-300 transition-colors">
          ← Back to sign in
        </Link>
      </p>
    </>
